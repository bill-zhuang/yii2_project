...
class Menu extends \dmstr\widgets\Menu
{
    ...
    public function run()
    {
        ...
        /********自定义开始**********/
        $menuitems = Yii::$app->cache->get('menuitems' . Yii::$app->user->id);

        $items = $this->normalizeItems($this->items, $hasActiveChild);
        //判断 如果当前路由 不在 items 中 则使用 缓存的 item 保证左侧 菜单栏  不变
        $switch = false;
        $finditem = function($item) use (&$finditem, &$items, &$switch) {
        ...
        $finditem($items);
        if (!$switch && $menuitems) {
            $items = json_decode($menuitems,true);
        } else {
            Yii::$app->cache->set('menuitems' . Yii::$app->user->id, json_encode($items));
        }
